...
int main(int argc, char** argv)
{
    ...
    ros::NodeHandle nh("~");
    readParameters(nh);
    cout<<"lidar_type: "<<lidar_type<<endl;//输出激光雷达类型
    ...
    p_imu->lidar_type = p_pre->lidar_type = lidar_type;//激光雷达类型
    p_imu->imu_en = imu_en;//是否使用IMU
    ...
    ros::Subscriber sub_pcl = p_pre->lidar_type == AVIA ? \
        ...
        nh.subscribe(lid_topic, 200000, standard_pcl_cbk);//激光点云
    ...
    ros::Publisher pubOdomAftMapped = nh.advertise<nav_msgs::Odometry> 
            ("/aft_mapped_to_init", 100000);//发布的里程计
...
//------------------------------------------------------------------------------------------------------
    ...
    while (status)
    {
        if (flg_exit) break;
        ros::spinOnce();
        if(sync_packages(Measures)) //同步处理
        {
            if (flg_first_scan)
            {
